...
class Late_AttU_Net(nn.Module):
    """
    Attention Unet implementation
    Paper: https://arxiv.org/abs/1804.03999
    """
    def __init__(self, img_ch=3, output_ch=1):
        super(Late_AttU_Net, self).__init__()
        ...
        self.Up_5 = up_conv(filters[4], filters[3] )
        self.Att_5 = Attention_block(F_g=filters[3], F_l=filters[3], F_int=filters[2] )
        self.Up_conv_5 = conv_block(filters[4], filters[3] )

        self.Up_4 = up_conv(filters[3], filters[2] )
        self.Att_4 = Attention_block(F_g=filters[2], F_l=filters[2], F_int=filters[1] )
        self.Up_conv_4 = conv_block(filters[3], filters[2] )

        self.Up_3 = up_conv(filters[2], filters[1] )
        self.Att_3 = Attention_block(F_g=filters[1], F_l=filters[1], F_int=filters[0] )
        self.Up_conv_3 = conv_block(filters[2], filters[1] )

        self.Up_2 = up_conv(filters[1], filters[0] )
        self.Att_2 = Attention_block(F_g=filters[0], F_l=filters[0], F_int=32 )
        self.Up_conv_2 = conv_block(filters[1], filters[0] )

        self.Conv_ = nn.Conv2d(filters[0], output_ch, kernel_size=1, stride=1, padding=0 )
...
class Dual_AttU_Net(nn.Module):
    """
    Attention Unet implementation
    Paper: https://arxiv.org/abs/1804.03999
    """
    def __init__(self, img_ch=3, output_ch=1):
        super(Dual_AttU_Net, self).__init__()
    ...
    def forward(self, x):
        ...
        d5_1 = self.Up_5(e5)
        x4_1 = self.Att_5(g=d5_1, x=e4)
        d5_1 = torch.cat((x4_1, d5_1), dim=1)
        d5_1 = self.Up_conv_5(d5_1)
    
        d4_1 = self.Up_4(d5_1)
        x3_1 = self.Att_4(g=d4_1, x=e3)
        d4_1 = torch.cat((x3_1, d4_1), dim=1)
        d4_1 = self.Up_conv_4(d4_1)
    
        d3_1 = self.Up_3(d4_1)
        x2_1 = self.Att_3(g=d3_1, x=e2)
        d3_1 = torch.cat((x2_1, d3_1), dim=1)
        d3_1 = self.Up_conv_3(d3_1)
        d2_1 = self.Up_2(d3_1)
        x1_1 = self.Att_2(g=d2_1, x=e1)
        d2_1 = torch.cat((x1_1, d2_1), dim=1)
        d2_1 = self.Up_conv_2(d2_1)
        out_1 = self.Conv_(d2_1)
...
#                img = e2[0,i,:,:].cpu().detach().numpy()*255
#                img = img.astype(np.uint8)
#                img = cv2.applyColorMap(img,2)
#                cv2.imwrite(r'E:\HuZhaoyu\DSP-main\visual\%s_e2_%d.png'%(mark,i),img)
               
#             e3 = self.Maxpool2(e2)
#             # print(e3.shape)
#             e3 = self.Conv3(e3)
    
#             for i in range(e3.shape[1]):
#                img = e3[0,i,:,:].cpu().detach().numpy()*255
#                img = img.astype(np.uint8)
#                img = cv2.applyColorMap(img,2)
#                cv2.imwrite(r'E:\HuZhaoyu\DSP-main\visual\%s_e3_%d.png'%(mark,i),img)
...
#                    img = d3[0,i,:,:].cpu().detach().numpy()*255
#                    img = img.astype(np.uint8)
#                    img = cv2.applyColorMap(img,2)
#                    cv2.imwrite(r'E:\HuZhaoyu\DSP-main\visual\%s_d3_%d.png'%(mark,i),img)
#             d2 = self.Up2(d3)
#             x1 = self.Att2(g=d2, x=e1)
#             d2 = torch.cat((x1, d2), dim=1)
#             for i in range(d2.shape[1]):
#                    img = d2[0,i,:,:].cpu().detach().numpy()*255
#                    img = img.astype(np.uint8)
#                    img = cv2.applyColorMap(img,2)
#                    cv2.imwrite(r'E:\HuZhaoyu\DSP-main\visual\%s_d2_%d.png'%(mark,i),img)
#         d2 = self.Up_conv2(d2)
...
class TransUNet(nn.Module):
    def __init__(self, img_dim=384, in_channels=12, out_channels=128, head_num=4, mlp_dim=512, block_num=8, patch_dim=16, class_num=4):
        super().__init__()
    ...
    def forward(self, x):
        x_embed, x1, x2, x3 = self.encoder(x)
        x = self.decoder(x_embed, x1, x2, x3)
