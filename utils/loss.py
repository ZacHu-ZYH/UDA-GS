...
class CrossEntropy2d(nn.Module):
    ...
    def forward(self, predict, target, weight=None):
        ...
        # predict 2 512 512 3 label 2 512 512
        # predict = predict[target_mask.view(n, h, w, 1).repeat(1, 1, 1, c)].view(-1, c).long()
        target = target.squeeze()
        loss = F.cross_entropy(predict+1e-8, target,reduction='mean')
        if weight==None:
            pass
        else:
